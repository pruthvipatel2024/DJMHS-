...
const TimetablePage: React.FC = () => {
  ...
  const [draftData, setDraftData] = useState<any | null>(
    StorageService.get('sdjm_timetable_draft', null)
  );

  useUnsavedWarning(!!draftData, 'You have an unapproved AI timetable draft. Are you sure you want to refresh?');
  ...
  const handleAutoGenerateDraft = async () => {
    ...
    try {
      ...
      StorageService.set('sdjm_timetable_draft', res.data.data);
    } catch (e: any) {
  ...
  useEffect(() => {
    const fetchMetadata = async () => {
      try {
        ...
        setDivisions(list);
        if (list.length > 0) {
          const savedDiv = StorageService.get('sdjm_timetable_division', '');
          const initialDiv = savedDiv && list.some(d => d.id === savedDiv) ? savedDiv : list[0].id;
